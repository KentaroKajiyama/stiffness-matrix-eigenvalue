...
class framework(object):
  ...
  def __init__(
    ...
    if isinstance(k, (int, float)):
      self.K = diags(k * np.ones(self.NB), format="csr")
    else:
      self.K = diags(k, format="csr")
    ...
    # まずスカラー演算を行ってからスパース行列との演算を実行
    scaled_seperation_norm = seperation_norm ** (power - 1)
    # self.tension = np.dot(self.K, scaled_seperation_norm)
    self.tension = self.K.diagonal() * scaled_seperation_norm
    self.KP = diags(self.tension / norm(self.dr, axis=1))

    ### effective stiffness to use in non-Hookean cases
    if power == 2:
      self.Ke = self.K
    else:
      self.Ke = diags(np.dot(self.K, seperation_norm ** (power - 2)))
  ...
  def rigidityMatrixSparse(self) -> np.ndarray:
...
def stiffness_matrix_sparce(p,bonds):
  F = framework(p,bonds)
  R = F.rigidityMatrixSparse()
  L = R.T @ R
  del F
  return L