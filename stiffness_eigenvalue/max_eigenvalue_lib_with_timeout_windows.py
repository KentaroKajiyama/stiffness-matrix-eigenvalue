...
def worker(L, k, which, tol, ncv, q):
  """multiprocessingで実行する固有値計算"""
  try:
    eigen_vals, eigen_vecs = eigsh(L, k=k, which=which, tol=tol, ncv=ncv)
    q.put((eigen_vals, eigen_vecs))
  except Exception as e:
    q.put(e)

def eigsh_with_timeout(L, k, which, tol, ncv, timeout_sec):
  """固有値計算をタイムアウト付きで実行"""
  q = multiprocessing.Queue()
  p = multiprocessing.Process(target=worker, args=(L, k, which, tol, ncv, q))
  p.start()
  ...
  if p.is_alive():
    p.terminate()
    p.join()
    return None, None  # タイムアウトした場合

  result = q.get()
  if isinstance(result, Exception):
    raise result  # 例外を再発生させる

  return result

def armijo(alpha, p, bonds, G, max_iter_for_armijo=None, timeout_sec=100):
  ...
  try:
    ...
    while True:
      eigen_vals, eigen_vecs = eigsh_with_timeout(L, k=8, which='SM', tol=1e-5, ncv=20, timeout_sec=timeout_sec)
      if eigen_vals is None:  # タイムアウト時の処理
        raise TimeoutError("eigsh computation timed out")
    ...
    while True:
      eigen_vals_after, _ = eigsh_with_timeout(L_after, NON_ZERO_INDEX+1, which='SM', tol=1e-5, ncv=20, timeout_sec=timeout_sec)
      if eigen_vals_after is None:  # タイムアウト時の処理
        raise TimeoutError("eigsh computation timed out")
    ...
    print("Timeout occurred while computing eigenvalues.")
    alpha = 0
